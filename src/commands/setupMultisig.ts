...
export async function setupMultisig(
  ...
  const currentMultisigBalance = await multisigProgram.provider.connection.getBalance(
    multisig.publicKey,
  )
  ...
  const txid = await multisigProgram.rpc.createMultisig(owners, new BN(threshold), nonce, {
    accounts: {
