...
export class MultisigSetOwnersAndChangeThreshold extends ProposalBase {
  constructor(public memo: string, public owners: PublicKey[], public threshold: number) {
    super(memo, owners)
  }

  async createInstr(ctx: MultisigContext): Promise<TransactionInstructionExt> {
    const multisigInstr = ctx.multisigProg.instruction.setOwnersAndChangeThreshold(
      this.owners,
